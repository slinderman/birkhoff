...
model distributions on the sphere's surface. Finally, the Placket-Luce model enables Bayesian inference of ranked data
\citep{Guiver2009bayesian}. However,this model arises from Luce axiom, a statement that brings the scenario of individualschoosing items. As here we are preoccupied with more general permutations, that assumption appears as against the grain \citep{luce2005individual}.
...
\begin{figure*}[ht]
  ...
  \caption{Results on the C.elegans inference example. (a) An example
    ...
    indicating that the sampling procedure is not deterministic.}
...
Gaussian noise.  Here, ~$X^{(j)}$ is a latent permutation of neurons
that must be inferred for each worm in order to align the observations
