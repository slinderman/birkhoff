...
\item Map~$M \to \mathsf{sinkhorn}(M)$, a point near the Birkhoff polytope, using the Sinkhorn-Knopp algorithm;
\item Set~${\Psi = \mathsf{sinkhorn}(M) + H \odot \Xi}$ where~$\odot$ denotes elementwise multiplication;
...
the~$\mathsf{sinkhorn}$ function poses no problems.  Had
we applied~$\mathsf{sinkhorn}$ directly to~$\Xi$, this would not be true.
...
this inverse is valid for all~$X \in \mcX_\tau$.\footnote{Consider a
  ...
  for points within~$\tau$ of a permutation, and the resulting density
...
\begin{multline*}
  q(X; M, \Sigma) = \\
  \frac{1}{\tau} \distNormal \left( \frac{1}{\tau}X -\frac{1-\tau}{\tau} \mathsf{round}(X); \, \mathsf{sinkhorn}(M), \Sigma \right),
\end{multline*}
for~${X \in \mcX_\tau}$.
