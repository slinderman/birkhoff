...
\item Map~$M \to \widetilde{M}$, a point in the Birkhoff polytope, using the Sinkhorn-Knopp algorithm;
\item Set~${\Psi = \widetilde{M} + V \odot \Noise}$ where~$\odot$ denotes elementwise multiplication;
...
\end{enumerate}
This procedure defines a mapping~${X = g_\tau(\Noise; \theta)}$ with~${\theta = \{M, V\}}$. When the elements
...
parameterized by~${\theta}$. Furthermore, as~$\tau$ goes to
...
the Sinkhorn-Knopp algorithm poses no problems.  Had
we applied this algorithm directly to~$\Noise$, this would not be true.
...
is~${g_\tau^{-1}(X; \theta) = \big(\tfrac{1}{\tau}X - \tfrac{1-\tau}{\tau}
  \mathsf{round}(X) - \widetilde{M}\big) \oslash V}$, where~$\oslash$ denotes elementwise division.  A slight wrinkle arises from the fact that
...
\begin{figure*}[ht] 
   \centering
   \includegraphics[width=6.5in]{../figures/figure8.pdf}
