...
\citep{jang2016categorical} proposed a method for discrete variational
inference based on the following observation: one-hot
vectors~${x \in \{0,1\}^N}$ can alternatively be viewed as vertices of
the simplex~$\Delta_N$; likewise, discrete probability mass
functions~$q(x; \theta)$ can be seen as atomic densities on the
...
vertices. Figure~\ref{fig:transforms}a and b illustrate this
idea. Real-valued random variables, e.g. Gumbel random variates,
are mapped through a temperature-controlled softmax function,
